...
class Controller:
    ...
    def handleRicorsione(self, e):
        ...
        bestpath, bestscore = self._model.getBestPath(nodoId)
        self._view.txt_result.controls.clear()
        self._view.txt_result.controls.append(
            ft.Text(f"Trovato un cammino che parte da {nodoId} "
                    f"con somma dei pesi uguale a {bestscore}."))

        for v in bestpath:
            self._view.txt_result.controls.append(ft.Text(f"{v[0]} -> {v[1]}: {v[2]}"))
        self._view.update_page()
